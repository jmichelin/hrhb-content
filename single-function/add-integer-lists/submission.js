const addIntegerLists = (listA, listB) => {
  ...
  let workingListA = listA;
  let workingListB = listB;
  let curr = dummyHead;
  let carry = 0;

  while (workingListA || workingListB || carry) {
    const valA = workingListA ? workingListA.val : 0;
    const valB = workingListB ? workingListB.val : 0;
    ...
    curr = curr.next;
    if (workingListA) workingListA = workingListA.next;
    if (workingListB) workingListB = workingListB.next;
  }
...
/*
// Strategy: iterate both number-lists pairwise, summing each pair of digits plus any carry from the
 previous pair, building a new number-list as we go.
